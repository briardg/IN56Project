		...
		try{
			if(((String)request.getParameter("villeD")).isEmpty()
				...
				String sql="select * " +
						   ...
						   " and (date_depart between str_to_date('"+(String)request.getParameter("dateD")+"','%d/%m/%Y')" +
						   " and (str_to_date('"+(String)request.getParameter("dateD")+"','%d/%m/%Y') + INTERVAL 1 DAY)) ";
			    if(!((String)request.getParameter("dateR")).isEmpty()){
			    	sql+=" and (date_retour between str_to_date('"+(String)request.getParameter("dateR")+"','%d/%m/%Y')" +
			    		 " and (str_to_date('"+(String)request.getParameter("dateR")+"','%d/%m/%Y') + INTERVAL 1 DAY))";
			    }
